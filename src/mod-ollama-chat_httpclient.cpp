...
std::string OllamaHttpClient::Post(const std::string &url,
  ...
  try {
    ...
    if (g_DebugEnabled) {
      LOG_INFO("server.loading",
               "[Ollama Chat] HTTP Request - Protocol: {}, Host: {}, Port: {}, "
               "Path: {}",
    ...
    if (protocol == "https") {
      ...
      if (g_DebugEnabled) {
        LOG_INFO("server.loading",
                 "[Ollama Chat] Using SSL client for HTTPS connection");
      }
      ...
      if (host.find("ngrok") != std::string::npos ||
          host.find("ngrok-free.app") != std::string::npos) {
      ...
      // Add Authorization header if provided (e.g. for OpenRouter Bearer token)
      if (!authHeader.empty())
      ...
      LOG_ERROR("server.loading",
                "[Ollama Chat] HTTPS requested but SSL support not available.");
      LOG_ERROR("server.loading",
                "[Ollama Chat] Please rebuild with OpenSSL support enabled.");
      LOG_ERROR("server.loading", "[Ollama Chat] See CMake output for OpenSSL "
    ...
    if (!response) {
      LOG_ERROR("server.loading",
                "[Ollama Chat] HTTP request failed - no response from {}:{}{}",
                host, port, path);
      return "";
    }
    ...
    if (response->status != 200) {
      LOG_ERROR("server.loading",
                "[Ollama Chat] HTTP request failed with status: {} for {}:{}{}",
                response->status, host, port, path);
      if (g_DebugEnabled) {
        LOG_INFO("server.loading", "[Ollama Chat] Response body: {}",
                 response->body);
      }
      return "";
    }
    ...
    if (g_DebugEnabled) {
      LOG_INFO("server.loading",
               "[Ollama Chat] HTTP request successful, response length: {}",
               response->body.length());
    }

    return response->body;
  } catch (const std::exception &e) {
    LOG_ERROR("server.loading", "[Ollama Chat] HTTP client exception: {}",
              e.what());
    return "";
  }
}

void OllamaHttpClient::SetTimeout(int seconds) { m_timeout = seconds; }
